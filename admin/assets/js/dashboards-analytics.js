...
(function () {
  ...
  // --------------------------------------------------------------------
    let productVisitCount = parseInt($('#productVisitCount').val());
    let postVisitCount = parseInt($('#postVisitCount').val());
    let categoryVisitCount =parseInt($('#categoryVisitCount').val());
    console.log(productVisitCount);
  const impressionDonutChartEl = document.querySelector('#impressionDonutChart'),
    impressionDonutChartConfig = {
      ...
      series: [productVisitCount, postVisitCount, categoryVisitCount],
      labels: ['محصولات', 'مقالات', 'دسته‌بندی'],
      stroke: {
        width: 0,
        lineCap: 'round'
      },
